...
SingleLayerKVConfig prepare_single_layer_kv_config(
    const torch::Tensor &lmc_dst_cache, const torch::Tensor &vllm_k_cache,
    const torch::Tensor &slot_mapping_full, bool direction, bool token_major,
    const torch::Tensor *vllm_v_cache) {

  SingleLayerKVConfig config;

  config.dims.num_tokens = slot_mapping_full.size(0);
  ...
  config.ub_params.scalar_type = vllm_k_cache.scalar_type();
  config.ub_params.slot_type = slot_mapping_full.scalar_type();
  config.direction = direction;
