...
MultiLayerKVConfig prepare_multi_layer_kv_config(
    ...
    config.page_buffer_ptrs = get_kernel_ptr<uint8_t, const torch::Tensor>(key_value_ptrs);
    config.slot_mapping_ptr = get_kernel_ptr<uint8_t, const torch::Tensor>(slot_mapping);
    ...
    config.kvcache_format = static_cast<kvcache_ops::KVCacheFormat>(kvcache_format_raw);
...
SingleLayerKVConfig prepare_single_layer_kv_config(
    ...
    config.lmc_cache_ptr = get_kernel_ptr<uint8_t, const torch::Tensor>(lmc_cache);
    config.vllm_cache_ptr = get_kernel_ptr<uint8_t, const torch::Tensor>(vllm_cache);
    config.slot_mapping_ptr = get_kernel_ptr<uint8_t, const torch::Tensor>(slot_mapping);
...
void compute_multi_layer_ub_params(
    MultiLayerKVConfig& config,
    const torch::Tensor& key_value,
    const torch::Device& paged_memory_device,
    const torch::Tensor& key_value_ptrs
    ...
    auto ascendcPlatform = platform_ascendc::PlatformAscendCManager::GetInstance(config.socName);
    uint64_t ubSize;
    ...
    int64_t baseBuffSize = numBuffsOnDev * config.hidden_dims * key_value.element_size();
    
    if (ubSize < static_cast<uint64_t>(baseBuffSize)) {
        std::string errStr = "Per TokenBuffer Size: " + std::to_string(baseBuffSize) + 
                            " exceeds UB Size: " + std::to_string(ubSize);
        PyErr_SetString(PyExc_RuntimeError, (errStr + " Please contact us.").c_str());
        throw py::error_already_set();
    }

    // step 2. work out how many tokens per loop
    config.maxTokensPerLoop = static_cast<int32_t>(ubSize / baseBuffSize) - 1; // Subtract 1 to provide a safety margin and avoid over-allocating the UB buffer, ensuring we do not exceed hardware limits due to possible rounding or small additional allocations.
    config.maxTokensPerLoop = std::min(config.maxTokensPerLoop, static_cast<int32_t>(config.num_tokens));
    
    // step 3. double check whether the perloop buffer can accommodate everything
    int64_t totalPerLoopBuffer = static_cast<int64_t>(config.maxTokensPerLoop) * baseBuffSize;
    if (ubSize < static_cast<uint64_t>(totalPerLoopBuffer)) {
        std::string errStr = "Per Loop Buffer Size: " + std::to_string(totalPerLoopBuffer) + 
                            " exceeds UB Size: " + std::to_string(ubSize);
        PyErr_SetString(PyExc_RuntimeError, (errStr + " Please contact us.").c_str());
        throw py::error_already_set();
    }
...
void compute_single_layer_ub_params(
    ...
    uint64_t ubSize;
    ascendcPlatform->GetCoreMemSize(platform_ascendc::CoreMemType::UB, ubSize);
    config.aiv_num = static_cast<uint32_t>(std::min(4, static_cast<int>(config.num_tokens)));
    ...
    config.max_tokens_per_loop = std::min(config.max_tokens_per_loop, config.num_tokens);
}
