...
// NOTE (Gingfung): This function encapsulates the logic of calling MindSpore's Python API
// The reason we had to do this were because when I directly included the mindspore runtime apis for building,
// it kept erroring with linking issues. I was in touch with a couple of MindSpore guys but never really got it working.
// Anyway, we will revisit using Mindspore runtime API once we are on 2.7.
int32_t _get_device_id_from_mindspore_hal_python_api_impl() {
    ...
    try {
        ...
        // NOTE: Mindspore current_device_id starts from 0 no matter what, so we should shift the device id 
        // iff users specify the ASCEND_RT_VISIBLE_DEVICES
        ...
        std::cerr << "C++ Internal: Python error in _get_device_id_from_mindspore_hal_python_api_impl: " << e.what() << std::endl;
        PyErr_Print(); // Print Python traceback
        device_id = -1;
    } catch (const std::exception &e) {
        std::cerr << "C++ Internal: Standard C++ exception in _get_device_id_from_mindspore_hal_python_api_impl: " << e.what() << std::endl;
        device_id = -1;
    }
    return device_id;
}
...
int8_t GetDeviceIdx() {
    ...
    if (devId == -1) {
        throw std::runtime_error("Failed to get device ID from MindSpore's Python API.");
    }
    return devId;
};
} // namespace framework_hal