...
HcclResult HcclAgent::Init() {
    ...
    // TODO: should pass the logical devId to the second argument of NetInit and NetOpenDev, but
    // Hccl currently wrongly passes the logical ID to halHostRegister, making it fail
    // when ASCEND_RT_VISIBLE_DEVICES is set. As a workeround, we temporarily pass the phyId
    // until this is fixed in upstream. We also temporarily don't check for HcclNetInit
    // errors since when passing phyId for both arguments, it throws an error if already 
    // initialized before.
    HcclNetInit(NICDeployment::NIC_DEPLOYMENT_DEVICE, phyId_, phyId_, false); 
    HCCL_CHECK(GetLocalIpv4(phyId_, localIp_));
    HCCL_CHECK(HcclNetOpenDev(&nicNetDevCtx_, NicType::DEVICE_NIC_TYPE, phyId_, phyId_, localIp_));
...
HcclResult HcclAgent::RegisterMem(void* ptr, uint64_t size, hccl::TransportMem::RmaMemDesc& memHandle) {
    std::lock_guard<std::mutex> lock(agentMutex_);
    ...
    if (it != registeredMems_.end()) {
        ...
        mem.addr = ptr;
        mem.size = size;
        if (attributes.memoryType == RT_MEMORY_TYPE_HOST || attributes.memoryType == RT_MEMORY_TYPE_USER) {
