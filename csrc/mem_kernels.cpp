...
void multi_layer_kv_transfer(torch::Tensor& key_value, // [kv, num_layer, num_tokens, hidden]
    ...
    compute_multi_layer_ub_params(config, key_value, paged_memory_device, key_value_ptrs);
    ...
    cmd.SetCustomHandler([config, key_value_ptr]() -> int {
        ...
        kvcache_ops::multi_layer_kv_transfer_kernel_v2(
            dtype_num, slot_num, config.kvcache_format,
            config.aiv_num, config.stream,
            config.page_buffer_ptrs, key_value_ptr,
            config.slot_mapping_ptr, config.hidden_dims,
            config.kv_size, config.num_layers, config.page_buffer_size,
            config.num_tokens, config.singlePerLoopBuffer,
...
void fused_multi_layer_kv_transfer(
    ...
    MultiLayerKVConfig config = prepare_multi_layer_kv_config(
        ...
        direction, use_mla, kvcache_format_raw
    );
    ...
    cmd.SetCustomHandler([config, staging_cache_ptr, key_value_ptr, cpu_buffer_size]() -> int {
        auto slot_num = vllm_ascend::get_dtype_from_torch(config.slot_type);
        auto dtype_num = vllm_ascend::get_dtype_from_torch(config.scalar_type);
        ...
        if (isH2D) {
            ret = aclrtMemcpyAsync(
                staging_cache_ptr, cpu_buffer_size,
                key_value_ptr, cpu_buffer_size,
                ACL_MEMCPY_HOST_TO_DEVICE, config.stream
            );
        ...
        kvcache_ops::multi_layer_kv_transfer_kernel_v2(
            dtype_num, slot_num, config.kvcache_format,
            config.aiv_num, config.stream,
            config.page_buffer_ptrs, staging_cache_ptr,
            config.slot_mapping_ptr, config.hidden_dims,
            config.kv_size, config.num_layers, config.page_buffer_size,
            config.num_tokens, config.singlePerLoopBuffer,
            config.maxTokensPerLoop, config.direction
        ...
        if (!isH2D) {
            ret = aclrtMemcpyAsync(
                key_value_ptr, cpu_buffer_size,
                staging_cache_ptr, cpu_buffer_size,
                ACL_MEMCPY_DEVICE_TO_HOST, config.stream
            );
            TORCH_CHECK(ret == ACL_ERROR_NONE,
...
void single_layer_kv_transfer(
    ...
    SingleLayerKVConfig config = prepare_single_layer_kv_config(
        ...
        direction, token_major, vllm_two_major
    );
    ...
    compute_single_layer_strides(config, lmc_key_value_cache, vllm_key_value_cache, 
    ...
    cmd.SetCustomHandler([config]() -> int {
        ...
        kvcache_ops::single_layer_kv_transfer_kernel_v2(
            ...
            config.vllm_block_stride, config.vllm_value_offset, config.vllm_buffer_size,
            config.lmc_token_stride, config.lmc_value_offset, config.lmc_buffer_size,
            config.max_tokens_per_loop, config.num_heads, config.head_dims,
            config.num_tokens, config.block_size, config.direction, config.token_major
        );
        return 0;
    });
    cmd.Run();
...
void batched_fused_single_layer_kv_transfer(
    ...
    SingleLayerKVConfig config = prepare_single_layer_kv_config(
        staging_cache, vllm_key_value_cache, slot_mapping_full,
        direction, token_major, vllm_two_major
    );
    ...
    cmd.SetCustomHandler([
        config, num_chunks, lmc_ptrs, lmc_copy_sizes, chunk_offsets,
        bytes_per_token, staging_v_plane_offset, lmc_v_offsets
    ]() -> int {
        auto slot_num = vllm_ascend::get_dtype_from_torch(config.slot_type);
        auto dtype_num = vllm_ascend::get_dtype_from_torch(config.scalar_type);
        aclError ret;
        ...
        if (!config.direction) {
            ...
            for (size_t i = 0; i < num_chunks; ++i) {
                uint8_t* staging_base = config.lmc_cache_ptr + 
                                       chunk_offsets[i] * bytes_per_token;
                ...
                if (config.token_major) {
                    ...
                    TORCH_CHECK(ret == ACL_ERROR_NONE, 
                               "H2D memcpy failed for chunk ", i, ", ret=", ret);
                } else {
                    // K plane
                    ret = aclrtMemcpyAsync(
                        staging_base, chunk_kv_bytes,
                    ...
                    TORCH_CHECK(ret == ACL_ERROR_NONE, 
                               "H2D memcpy (K) failed for chunk ", i, ", ret=", ret);
                    ...
                    TORCH_CHECK(ret == ACL_ERROR_NONE, 
                               "H2D memcpy (V) failed for chunk ", i, ", ret=", ret);
                }
            }
        ...
        // from_gpu (paged -> staging -> CPU)
            ...
            for (size_t i = 0; i < num_chunks; ++i) {
                ...
                if (config.token_major) {
                    ...
                    TORCH_CHECK(ret == ACL_ERROR_NONE, 
                               "D2H memcpy failed for chunk ", i, ", ret=", ret);
                } else {
                    // K
                    ret = aclrtMemcpyAsync(
                        lmc_ptrs[i], chunk_kv_bytes,
                    ...
                    TORCH_CHECK(ret == ACL_ERROR_NONE, 
                               "D2H memcpy (K) failed for chunk ", i, ", ret=", ret);
                    ...
                    TORCH_CHECK(ret == ACL_ERROR_NONE, 
                               "D2H memcpy (V) failed for chunk ", i, ", ret=", ret);
                }
            }
        }
