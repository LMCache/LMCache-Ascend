...
def plot_bar_metric(
    ...
    mean_df['category'] = pd.Categorical(mean_df['category'], categories=CATEGORY_ORDER, ordered=True)
    ...
    sns.barplot(
        x="scenario", y=metric_col, hue="category", 
        data=mean_df, ax=ax, 
...
def analyze_benchmark_results(csv_paths: List[str], results_dir: str, scenario_map: Dict[str, str]):
    ...
    for csv_path in csv_paths:
        ...
        if scenario in file_paths:
             print(f"Warning: Scenario '{scenario}' already mapped! Skipping file {filename}.")
             continue
    ...
    for scenario, path in file_paths.items():
        try:
            ...
            latency_col_found = next((col for col in LATENCY_CANDIDATES if col in df_full.columns), None)
            throughput_col_found = next((col for col in THROUGHPUT_CANDIDATES if col in df_full.columns), None)
            ...
            if not required_cols:
                 print(f"Error loading {path}: No required columns (quality, latency, or throughput metrics) found.")
                 continue
    ...
    print(df.groupby(["category", "scenario"])[analysis_cols].agg([
    ...
    if FINAL_THROUGHPUT_COL in df.columns:
        metrics_to_plot.append((FINAL_THROUGHPUT_COL, "Throughput", "Throughput (req/s)"))
    ...
    fig.suptitle("Benchmark Core Metrics Analysis: Average Performance Comparison", fontsize=16)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--scenario-map', 
        nargs='+', 
        default=['lmcache_offline:LMCache (No Blend) Offline', 
                 'cacheblend_offline:CacheBlend Offline',
                 'lmcache_online:LMCache (No Blend) Online',
                 'cacheblend_online:CacheBlend Online'], 
        help='Map filenames identifiers to scenario display names (e.g., "lmcache_offline:LMCache (No Blend) Offline")'
    )
    ...
    for item in args.scenario_map:
        try:
            ...
            print(f"Error: Invalid scenario map format '{item}'. Use 'identifier:DisplayName'.")
            exit(1)
