...
def get_allocator(device_id: int, kv_shape: Tuple[int, ...], dtype: torch.dtype, use_host: bool) -> PagedCpuGpuMemoryAllocator:
    ...
    allocator.init_gpu_memory_allocator(
        buffer_size, [torch.Size(kv_shape)], [dtype], MemoryFormat.KV_2LTD, device_id,
    )
    
    if use_host:
        allocator.init_cpu_memory_allocator(
            buffer_size, [torch.Size(kv_shape)], [dtype], MemoryFormat.KV_2LTD,
        )
    return allocator
...
def sender_process(
    ...
    try:
        ...
        allocator = get_allocator(config.send_device_id, config.kv_shape, config.dtype, config.use_host_memory)
        ...
        for i in range(config.num_objs):
            obj = allocator.allocate(torch.Size(config.kv_shape), config.dtype, fmt=MemoryFormat.KV_2LTD, allocator_type=alloc_type)
        ...
        # Wait for Receiver to also be initialized
        # This prevents the Sender from writing before the Receiver's background thread is ready
        wait_start = time.time()
        while "receiver_init_done" not in shared_dict:
            time.sleep(0.1)
            if time.time() - wait_start > 30:
                raise TimeoutError("Sender timed out waiting for Receiver initialization")
...
def receiver_process(
    ...
    try:
        ...
        allocator = get_allocator(config.recv_device_id, config.kv_shape, config.dtype, config.use_host_memory)
        alloc_type = "cpu" if config.use_host_memory else "gpu"
        ...
        while "sender_init_done" not in shared_dict:
            time.sleep(0.1)
            if time.time() - wait_start > 30:
                raise TimeoutError("Receiver timed out waiting for Sender initialization")
        ...
        for i, obj in enumerate(objs):
            ...
            if not is_equal:
                ...
                logger.error(f"Mismatch in object {i}. Expected {expected_val}, got sample: {sample}")
...
def run_hccl_test(config: HcclTestConfig):
    ...
    with mp.Manager() as manager:
        ...
        p_recv = mp.Process(target=receiver_process, args=(config, shared_dict, start_event), name="ReceiverProcess")
        p_send = mp.Process(target=sender_process, args=(config, shared_dict, start_event), name="SenderProcess")
...
@pytest.mark.skipif(not torch.npu.is_available() or torch.npu.device_count() < 2, reason="Requires at least 2 NPU devices")
@pytest.mark.parametrize("num_objs, num_layer, chunk_size, num_kv_head, head_size", [
    (2,  31, 256, 8, 128),
    (10, 31, 256, 8, 128),
])
