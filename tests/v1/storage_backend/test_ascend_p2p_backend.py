...
def _make_p2p_backend_stub(
    ...
    * ``save_unfull_chunk=True`` (last chunk): calls ``_get_unfull_chunk_shapes``.
    ...
    if kv_shapes is None:
        kv_shapes = [torch.Size([2, 2, 256, 512])]
...
class TestAscendP2PBackendUnit:
    ...
    def test_allocate_memory_for_keys_npu_path(self):
        """NPU path uses memory_allocator.gpu_allocator.allocate, not local_cpu_backend."""
        backend = MagicMock()
    ...
    def test_batched_get_non_blocking_push_mode(self, async_loop):
        ...
        backend = _make_p2p_backend_stub(pull_mode=False, delay_pull=False, use_npu=False)
        backend.loop = async_loop
        backend.lookup_id_to_peer_mapping = {
            "lu_push": ("target_peer_url", "cpu")
        }
    ...
    def test_handle_pull_mode_transfer_read_failure_still_sends_done(
        self, async_loop
    ):
