...
def test_fused_multi_layer_kvcache_merged_fmt(
    ...
    """
       - from_gpu: Verify that the output of the fused method is consistent with the baseline method.
       - to_gpu: Verify that the KV cache content is correct after data is written back.
    """
    device = "npu"
    num_blocks = 256
    dtype = torch.bfloat16
...
def test_batched_fused_single_layer_kernel(
    num_tokens, num_layers, num_chunks,
    ...
    kv_cache_src = generate_kv_cache_paged_list_tensors(
        num_blocks, device, num_layers, num_heads, head_size,
        block_size, dtype, vllm_two_major=vllm_two_major
    )
    ...
    try:
        ...
        print(f"\n[from_gpu] Baseline: {baseline_from_time:.3f} ms, Fused: {fused_from_time:.3f} ms")

        # 验证 from_gpu 正确性
        for layer_id in range(num_layers):
            for chunk_id in range(num_chunks):
                assert torch.equal(cpu_baseline[layer_id][chunk_id], cpu_fused[layer_id][chunk_id]), \
        ...
        print(f"[to_gpu] Baseline: {baseline_to_time:.3f} ms, Fused: {fused_to_time:.3f} ms")

        check_paged_kv_cache_equal(
            kv_cache_src, kv_cache_dst_baseline, slot_mapping_full,
            num_heads=num_heads, head_size=head_size, vllm_two_major=vllm_two_major
        )

        check_paged_kv_cache_equal(
            kv_cache_src, kv_cache_dst_fused, slot_mapping_full,
            num_heads=num_heads, head_size=head_size, vllm_two_major=vllm_two_major
        )
