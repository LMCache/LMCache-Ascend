...
def MooncakeStoreConnector__register_cpu_buffer(self):
    ...
    try:
        ...
        if hasattr(allocator, "pin_allocator") and hasattr(
            ...
            if result == 0:
                logger.info(
                    f"Registered: {hex(get_data_ptr(buffer))}, {get_numel(buffer)} bytes"
                )
            else:
...
async def MooncakeStoreConnector__put_without_metadata(self, key_str: str, memory_obj: MemoryObj):
    """
