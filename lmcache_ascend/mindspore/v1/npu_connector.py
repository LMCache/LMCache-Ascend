...
#         kvsize = 1 if self.use_mla else 2

#         # NOTE: Mindspore has no index from the torch tensor API
#         # will use mindspore.hal in the future, 
#         # so we only use 1 tensor and manage for its own device
#         self.kv_cache_pointers_on_gpu = torch.empty([num_layers, kvsize],
#                                                 dtype=torch.int64,
#                                                 device='Ascend',
#                                                 pin_memory=True)
#         self._initialized_kv_ptrs = False
...
#     def _initialize_pointers(self, kv_caches: List[torch.Tensor]) -> torch.Tensor:
#         # if self._initialized_kv_ptrs:
#         #     return self.kv_cache_pointers_on_gpu

#         device = kv_caches[0][0].device
#         assert device.type == "Ascend", "The device should be Ascend NPU."

#         kv_cache_pointers_cpus = torch.Tensor([[kvc.data_ptr() for kvc in layer_kv ]for layer_kv in kv_caches])
#         self.kv_cache_pointers_on_gpu.copy_(kv_cache_pointers_cpus, non_blocking=False)
#         torch.cuda.synchronize()

#         self._initialized_kv_ptrs = True

#         # if self.use_mla: kv_caches[0][0].shape: [num_pages, page_size, head_size]
#         # else: kv_caches[0][0].shape: [num_pages, page_size, num_heads, head_size]
#         # MINDSPORE store k and v separately
