...
MODEL="${MODEL:-/xxx/models/Qwen3.5-122B-A10B}"
...
export LMCACHE_CONFIG_FILE="$SCRIPT_DIR/lmcache_qwen35.yaml"

mkdir -p "$WORK_DIR/logs/qwen35"
LOG_FILE="$WORK_DIR/logs/qwen35/lmcache-mtp3-graph-$(date +%Y%m%d-%H%M%S).log"
printf '%s\n' "$LOG_FILE" > "$SCRIPT_DIR/latest-qwen35-log.txt"
echo "Model: $MODEL; BF16 TP8; MTP3; target FULL_DECODE_ONLY; draft eager"
