# GLM-5.1 (w8a8) Single-Node Deployment Guide — DDR + SSD

Deploy GLM-5.1-w8a8 as a single vLLM service on one Ascend server with
...
GLM-5.1-w8a8 weights exist under `/mnt/sdb/models/GLM-5.1-w8a8`.
...
local_disk: "/mnt/lmcache_ssd_dir"      # SSD cache path inside the container
...
vllm serve /mnt/sdb/models/GLM-5.1-w8a8 \
  --host 0.0.0.0 \
  --port 8077 \
...
curl http://localhost:8077/v1/chat/completions \
  ...
  -d '{"model":"glm-51","messages":[{"role":"user","content":"Hello"}],"max_tokens":64}'
```
